...
{
    // YOUR CODE HERE
  int heightMax = this->root->height;
  AvlNode* t = this->root;
  int depth = heightMax - t->height;
  int length = int_path_length(t, depth);
  double expectedLength = (double)(length) / num_nodes;
  return expectedLength;  // stub, remove after writing your code
}
...
int AvlTree::int_path_length(AvlNode *t, int depth) {
  ...
  if (t->left == NULL && t->right == NULL) {
    ...
    if (t->left != NULL){
      leftLength = int_path_length(t->left, depth+1);
    }
    if (t->right != NULL){
      rightLength = int_path_length(t->right, depth+1);
    }
    return leftLength + rightLength + depth;
  }
  return -1; // put your actual return value here when you write this function
}
